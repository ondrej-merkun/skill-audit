...
## Corpus Check
- 184 files · ~149,428 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 757 nodes · 1405 edges · 25 communities detected
...
3. `readFile()` - 21 edges
4. `parse()` - 21 edges
5. `runExplain()` - 17 edges
6. `add()` - 15 edges
7. `renderTableToString()` - 15 edges
8. `computeTreeSha256()` - 14 edges
9. `runRules()` - 12 edges
...
Cohesion: 0.05
Nodes (51): appendToIgnoreList(), getConfigDir(), getIgnoreListPath(), loadIgnoreList(), readIgnoreListContent(), formatAgentPath(), renderDetail(), renderEnrichment() (+43 more)
...
Cohesion: 0.05
Nodes (22): addReachablePath(), buildReachabilityIndex(), classifySupportingFileRole(), extractSnippet(), findingForRole(), globalPattern(), hasNestedQuantifier(), isMarkdownPromptFile() (+14 more)
...
Cohesion: 0.15
Nodes (19): ancestorDirsToGitRoot(), descendantWindsurfRulesDirs(), discoverRuleFile(), gitRootFromAncestors(), hasNestedWorkspaceRules(), hasWorkspaceRules(), isDirectory(), isRegularFile() (+11 more)
...
- **Thin community `Community 19`** (5 nodes): `Git workflow helper — generates branch names and commit messages.`, `recent_commits()`, `staged_summary()`, `suggest_branch()`, `suggest.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (5 nodes): `extract_tables()`, `extract_text()`, `get_metadata()`, `Extract text and tables from PDF files.`, `extractor.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (4 nodes): `list_buckets()`, `_load_creds()`, `upload_file()`, `aws_ops.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (3 nodes): `notify.py`, `notify_build()`, `notify_deploy()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (3 nodes): `kanban.py`, `add_task()`, `list_tasks()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (2 nodes): `Template usage notes.  Local setup:     pip install example-sdk fastapi uvicorn`, `template.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `trim()` connect `Community 8` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 6`, `Community 9`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `add()` connect `Community 11` to `Community 0`, `Community 2`, `Community 3`, `Community 4`, `Community 10`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `parse()` connect `Community 2` to `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 9`, `Community 12`?**
...
- **Are the 18 inferred relationships involving `runScan()` (e.g. with `formatSupportedAgentIds()` and `loadSelectedLlmConfigs()`) actually correct?**
  _`runScan()` has 18 INFERRED edges - model-reasoned connections that need verification._
